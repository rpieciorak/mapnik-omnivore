  ...
  */
  ...
  q.defer(function(next) {
    _this.getExtent(function(err, extent) {
      // disable dynamic zoom for rasters for now
      //minZoom = utils.getDynamicMinZoom(validSpatialResolutions, extent, tileSize);
      next();
    });
  });

  return q.await(function(err) {
    callback(
      err,
      Math.max(0, minZoom || (validSpatialResolutions.length - 6)),
